...
一条给 2:1 等距游戏用的预渲染管线——以及用这条管线做出来的一个完整种田游戏。
...
它特别的地方在于：图集里的每个精灵除了颜色，还带一层法线和一层材质；而**逐像素深度是从投影里算出来的，不是另外烘的**。于是遮挡变成硬件深度测试，「谁盖住谁」不再是一个排序问题；而光照是一层连续的场，盖在硬像素上面。
...
```
src/            13 个运行时模块——游戏本体。按 ES 模块加载，没有构建步骤。
